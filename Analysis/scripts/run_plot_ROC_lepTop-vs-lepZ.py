...
commonCut_bkg = "({jet}_pT_reco > 400) && (abs({jet}_eta_reco) < 2.5) && ({jet}_nConsti_reco >= 1) && ({jet}_nearestGenZDR_reco < 0.6) && ({jet}_nearestGenZIsLeptonic_reco > 0.5) && ({jet}_nearestGenZlep1DR_reco < 0.6) && ({jet}_nearestGenZlep2DR_reco < 0.6)"
...
for pdgidIdx in range(0, len(l_pdgid)) :
    ...
    if (era == "2018") :
        ...
        l_curveInfoUpdate_sample.append({
            "label": f"t^{{{pdgidTex}}} (t#bar{{t}}) vs. Z^{{lep}} (Z+jets)",
            "sig": {"sample": "TTJets"},
            "bkg": {"sample": "DYJets"},
            "color": 2,
        })
    ...
    l_curveInfoUpdate_sample.append({
        "label": f"t^{{{pdgidTex}}} (Z'_{{2 TeV}}) vs. Z^{{lep}} (Z+jets)",
    ...
    l_curveInfoUpdate_sample.append({
        "label": f"t^{{{pdgidTex}}} (Z'_{{4 TeV}}) vs. Z^{{lep}} (Z+jets)",
    ...
    l_curveInfoUpdate_sample.append({
        "label": f"t^{{{pdgidTex}}} (W'_{{4 TeV}}) vs. Z^{{lep}} (Z+jets)",
    ...
    l_curveInfoUpdate_sample.append({
        "label": f"t^{{{pdgidTex}}} (W'_{{6 TeV}}) vs. Z^{{lep}} (Z+jets)",
    ...
    d_config = {
        ...
        "legendtextsize": 0.04,
        "cpufrac": 0.9,
        "outtag": f"{pdgidName}Top-vs-lepZ",
        "outdir": f"plots/ROCs/particle_net_mod1b_ak8_with-WZ_2022-05-11_18-55-28/{era}",
    }
