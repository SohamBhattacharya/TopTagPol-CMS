...
commonCut_bkg = "({jet}_pT_reco > 400) && (abs({jet}_eta_reco) < 2.5) && ({jet}_nConsti_reco >= 1) && ({jet}_nearestGenWDR_reco < 0.6) && ({jet}_nearestGenWIsLeptonic_reco > 0.5)"
...
for pdgidIdx in range(0, len(l_pdgid)) :
    ...
    if (era == "2018") :
        ...
        l_curveInfoUpdate_sample.append({
            "label": f"t^{{{pdgidTex}}} (t#bar{{t}}) vs. W^{{lep}} (W+jets)",
            "sig": {"sample": "TTJets"},
            "bkg": {"sample": "WJets"},
            "color": 2,
        })
    ...
    l_curveInfoUpdate_sample.append({
        "label": f"t^{{{pdgidTex}}} (Z'_{{2 TeV}}) vs. W^{{lep}} (W+jets)",
    ...
    l_curveInfoUpdate_sample.append({
        "label": f"t^{{{pdgidTex}}} (Z'_{{4 TeV}}) vs. W^{{lep}} (W+jets)",
    ...
    l_curveInfoUpdate_sample.append({
        "label": f"t^{{{pdgidTex}}} (W'_{{4 TeV}}) vs. W^{{lep}} (W+jets)",
    ...
    l_curveInfoUpdate_sample.append({
        "label": f"t^{{{pdgidTex}}} (W'_{{6 TeV}}) vs. W^{{lep}} (W+jets)",
    ...
    d_config = {
        ...
        "legendtextsize": 0.04,
        "cpufrac": 0.9,
        "outtag": f"{pdgidName}Top-vs-lepW",
