                ...
                for dataKey in list(d_branchName_alias.keys()) :
                    ...
                    for iBrKey, brKey in enumerate(list(d_branchName_alias[dataKey].keys())) :
                        ...
                        if (len(d_data_shape[dataKey]) == 3) :
                            ...
                            maxEntries = d_data_shape[dataKey][1]
                            
                            a_br_padded = awkward.pad_none(a_br, target = maxEntries, axis = 1, clip = True)
...
class ParticleDataset :
    ...
    def __post_init__(
        ...
        for key in self.l_particleFeatureKey :
            ...
            shape = (self.batchSize, self.d_loadConfig["maxParticles"], nFtr)
            self.d_data_shape[key] = shape
            
            inputShape = (self.d_loadConfig["maxParticles"], nFtr)
            self.d_data_inputShape[key] = inputShape
            
            outputShape = (None, self.d_loadConfig["maxParticles"], nFtr)
            self.d_data_outputShape[key] = outputShape
        
        for key in self.l_svFeatureKey :
            ...
            shape = (self.batchSize, self.d_loadConfig["maxSVs"], nFtr)
            self.d_data_shape[key] = shape
            
            inputShape = (self.d_loadConfig["maxSVs"], nFtr)
            self.d_data_inputShape[key] = inputShape
            
            outputShape = (None, self.d_loadConfig["maxSVs"], nFtr)
            self.d_data_outputShape[key] = outputShape
        ...
        if (self.jetFeatureKey is not None) :
            ...
            self.d_data_shape[self.jetFeatureKey] = (self.batchSize, nFtr)
            self.d_data_inputShape[self.jetFeatureKey] = (nFtr,)
            self.d_data_outputShape[self.jetFeatureKey] = (None, nFtr)
        
        
        self.reweightBrKey = "reweightBranches"
        
        if ("reweightBranches" in self.d_loadConfig) :
            
            nBr = len(self.d_loadConfig[self.reweightBrKey])
            
            self.d_data_shape[self.reweightBrKey] = (self.batchSize, nBr)
            #self.d_data_inputShape[self.reweightBrKey] = (nBr,)
            #self.d_data_outputShape[self.reweightBrKey] = (None, nBr)
        
        else :
            
            self.reweightBrKey = None
        
        
        self.queueSize_max = min(30, self.nBatch) * self.nMerge
    ...
    def batchGenerator(
        ...
        for iProc, proc in enumerate(self.l_process*self.nMerge) :
            ...
            isLast, procIdx, result = self.queue.get()
    ...
    def createBatch(
        ...
        for key in self.d_data_shape.keys() :
            ...
            if (key == self.reweightBrKey) :
                
                # Use any one of the categories to get the reweighting branches as that'll be the same for all
                catInfo = self.d_catInfo[list(self.d_catInfo.keys())[0]]
                d_branchName_alias[key] = catInfo.d_reweightBrName_alias
        ...
        a_weight = numpy.ones(len(indices))
        
        if (self.reweightBrKey is not None) :
            ...
            for sliceIdx_batch, sliceIdx_global in enumerate(a_sliceIdx) :
                ...
                i1 = sliceIdx_batch*self.sliceSize
                i2 = i1 + self.sliceSize
                
                a_axesVal = d_data[self.reweightBrKey][i1: i2, :].transpose()
                
                a_binVal = catInfo.reweightHistWeight.values(flow = True)[
                    catInfo.reweightHist.axes.index(*a_axesVal)
                ]
                
                a_weight[i1: i2] = a_binVal
        
        
        a_shf_idx_split = numpy.split(numpy.random.permutation(len(indices)), self.nMerge)
        
        
        for idx, a_shf_idx in enumerate(a_shf_idx_split) :
            
            d_data_cpy = sortedcontainers.SortedDict()
            
            for key in self.d_data_shape.keys() :
                
                # Exclude the reweighting branches. No need to return them as they're not used in the training.
                if (key == self.reweightBrKey) :
            ...
            isLast = bool(idx == (len(a_shf_idx_split)-1))
            
            self.queue.put(
                (isLast, procIdx, (d_data_cpy, a_label_tmp, a_weight_tmp))
            )
