...
class CustomCallback(tensorflow.keras.callbacks.Callback):
    ...
    dataset             : Any
    l_categoryLabel     : Any
    jetFeatureKey       : str               = None
    l_jetFeature        : List[str]         = None
    ...
    def __post_init__(
        ...
        self.writer = tensorflow.summary.create_file_writer(self.writerPath)
        ...
        d_data, a_label, a_weight = self.dataset
        
        if (self.jetFeatureKey is not None and len(self.jetFeatureKey) and self.jetFeatureKey in d_data) :
            
            print("Logging %s histograms..." %(self.jetFeatureKey))
            
            dataset_jetFtr = d_data[self.jetFeatureKey]
            
            for iFeature in range(0, dataset_jetFtr.shape[1]) :
                
                print("Logging %s%d histogram..." %(self.jetFeatureKey, iFeature))
                
                for cat in self.l_categoryLabel :
                    
                    nz_row = numpy.nonzero(a_label[:, cat])
                    
                    with self.writer.as_default():
                        
                        tensorflow.summary.histogram(
                            "jetFeature_%d" %(iFeature),
                            dataset_jetFtr[nz_row, iFeature],
                            step = cat,
                            buckets = 1000,
                            description = self.l_jetFeature[iFeature] if (self.l_jetFeature is not None) else "",
                        )
            
            print("Logged %s histograms." %(self.jetFeatureKey))
    
    
    def on_epoch_begin(self, epoch, logs = None) :
        
        print("[epoch %d] Learning rate:" %(epoch), self.model.optimizer.learning_rate)
