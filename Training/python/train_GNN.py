...
def main() :
    ...
    nMerge = d_loadConfig["nMerge"]
    batchSize = d_loadConfig["batchSize"]*nMerge
    sliceSize = d_loadConfig["sliceSize"]*nMerge
    ...
    if (l_svFeatureKey[0] in d_loadConfig) :
        
        for key in d_loadConfig :
    ...
    d_catInfo_tst = utils.load_catInfo_from_config(d_loadConfig, loadReweightInfo = False)
    
    for (iCat, cat) in enumerate(d_catInfo_trn.keys()) :
        
        if (d_catInfo_trn[cat].reweightHist is None) :
            
            continue
        
        d_catInfo_tst[cat].set_reweightInfo(
            reweightHist = d_catInfo_trn[cat].reweightHist,
            reweightHistWeight = d_catInfo_trn[cat].reweightHistWeight,
        )
    ...
    if (isinstance(d_loadConfig["learningRate"], float)) :
        
        learning_rate = d_loadConfig["learningRate"]
        
        lr_reduce_callback = tensorflow.keras.callbacks.ReduceLROnPlateau(
            monitor = "val_loss",
            factor = 0.1,
            patience = 3,
            verbose = 1,
    ...
    elif ("epoch" in d_loadConfig["learningRate"] and "rate" in d_loadConfig["learningRate"]) :
    ...
    if ("loadModel" in d_loadConfig) :
        ...
        #    from_logits = False,
        #    alpha = 1.0, # Default: 0.25
        #    gamma = 2.0, # Default: 2.0
        #    reduction = tensorflow.keras.losses.Reduction.AUTO,
        #)
    ...
    mylog_callback = callbacks.CustomCallback(
        ...
        jetFeatureKey = jetFeatureKey,
        l_jetFeature = d_loadConfig[jetFeatureKey] if (jetFeatureKey is not None) else None,
    )
